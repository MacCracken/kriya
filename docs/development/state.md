...
M1 closed at v0.2.0; M2 closed at v0.3.0; M3 closed at v0.4.0; M4 closed at v0.5.0; M5 closed at v0.6.0; M6 closed at v0.7.0; **M7 closed at v0.8.0**. **Thirty-eight shipped utilities** (six M1 + seven M2 + seven M3 + ten M4 + three M5 + five M6: `seq`, `env`, `date`, `du`, `df`); six shared lib modules in place (`exit`, `path`, `errmsg`, `args`, `fs`, `protected`); **eight** ADRs accepted (0001–0008); two architecture notes (signal model + errno policy); M7 POSIX audit at `docs/audit/2026-05-18-posix-compliance.md`. Cold-start re-benched at each release boundary: 1.185ms (v0.2.0) → 1.159ms (v0.3.0) → 1.208ms (v0.4.0) → 1.198ms (v0.5.0) → 1.192ms (v0.6.0) → 1.212ms (v0.7.0) → **1.201ms (v0.8.0)** — M7 added no dispatcher entries; the small variance is noise.
