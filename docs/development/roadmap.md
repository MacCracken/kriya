...
**Enabler:** the spec-renderer in `src/lib/help.cyr` (shipped 1.3.0), per [ADR 0002](../adr/0002-option-parsing-humans-and-agents.md)'s "Implementation status".
It already reads every utility's option table back out of `flags_new()` and carries the per-utility
prose (summary, synopsis, operands, exit codes, examples). ⚠ **The JSON form serialises that same
data — do not introduce a second source of truth for it.**
...
The cold-start budget matters here specifically: every `<tab>` may spawn one of these, against a 2 ms
target. ⚠ **The last good measurement is 1.185 ms, taken before 1.3.0.** 1.3.0's re-measure was
abandoned at load average 31 (contradictory A/B across a swap — noise, not signal), and the binary
has since grown +21 KB. Re-measure on a quiet box before 1.3.2 ships.
...
- **1.3.x — `--version`, and a consumer for `src/version_str.cyr`.** ⛔ **The version string is
  currently dead code.** `version_str.cyr` is auto-generated by `scripts/version-bump.sh`, included by
  `src/main.cyr`, and **read by nothing** — its own header names `kriya --version` as a consumer that
  was never written, so `kriya ls --version` answers "bad option". Every release has been regenerating
  a string no one can print. Cheap to fix; belongs in this arc because a completer that can enumerate
  utilities will also want to know which build it is talking to. ⚠ Decide whether `--version` is
  dispatcher-only (`kriya --version`) or accepted by every utility the way `--help` now is — GNU does
  the latter, and ADR 0002 would need a line either way.
