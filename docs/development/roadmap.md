...
**Enabler:** the renderers in `src/lib/help.cyr` — human form shipped 1.3.0, `--help=json` shipped
1.3.1 with the schema amended in [ADR 0002 Appendix A](../adr/0002-option-parsing-humans-and-agents.md#appendix-a---help=json-schema-v1).
Both read every utility's option table back out of `flags_new()` and share one set of per-utility
prose. ⚠ **`kriya --list` serialises that same data — do not introduce a second source of truth.**
...
target. ⚠ **Measured at 1.3.1 and flat** — interleaved A/B against a 1.3.0 binary, 500 pairs:
0.474 ms → 0.477 ms (+0.6%, noise). ⛔ **Report the release-over-release delta, not the absolute.**
The same run read ~180 µs above the 1.196 ms v1.0.0 baseline purely because the box was loaded, and
the reference binary must be named `kriya` or the dispatcher rejects it on `argv[0]` and the arm
measures a failed dispatch.
