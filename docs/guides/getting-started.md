...
cyrius build src/main.cyr build/kriya    # compile
...
- `src/test.cyr` — top-level test entry referenced by `cyrius.cyml [build].test`. Add unit cases here or in `tests/kriya.tcyr`.
...
2. Add a test case to `tests/kriya.tcyr`.
3. Run `cyrius test`.
