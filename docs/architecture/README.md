...
- [002 — Signal handling model](002-signal-handling-model.md) — kernel defaults everywhere (SIGPIPE → 141, SIGINT → 130); no utility installs a handler. `tee -i` / `-p` set `SIG_IGN` dispositions, and only on request ([ADR 0016](../adr/0016-tee-signal-dispositions.md)).
