...
- **Tests**: `tests/kriya.tcyr` gains 17 octal-mode-parser assertions (78/78 passing total). `scripts/smoke-mkdir.sh` 24/24, `scripts/smoke-rmdir.sh` 24/24, `scripts/smoke-touch.sh` 26/26, `scripts/smoke-ln.sh` 30/30 (symbolic + hard creation, `-f` overwrite, single-arg form, multi-into-dir, ADR-0003 deploy-retarget idiom `-s -f -n`, `-P` hard-link-the-symlink with inode-match verification, dangling symlink ok, hard-to-missing error, verbose output). `tests/kriya.bcyr` gains `args/parse_octal_mode` at **20ns** steady-state.
