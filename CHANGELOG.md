...
- **`cp`** (`src/cmd/cp.cyr`) — non-recursive POSIX `cp(1)` ship. Flags `-f`/`--force`, `-i`/`--interactive` (ADR 0002: when stdin is not a tty, exit 2 invocation-wide; when it is, prompt for `y`/`Y`), `-p`/`--preserve` (mode + atime/mtime; ownership preservation deferred — needs root to matter), `-v`/`--verbose`. Argument shapes: single-pair `cp SRC DST` and multi-into-dir `cp SRC... DIR/`. Self-copy refused (same `(st_dev, st_ino)` check). Directory sources without `-R` exit 1 with EISDIR. `-R`/`-r` and the ADR-0003 `-P`/`-H`/`-L` symlink-policy matrix deferred to the next ship (lands with `src/lib/fs.cyr` and the `openat(O_NOFOLLOW \| O_DIRECTORY)` traversal discipline). Copy loop uses a process-lifetime 64 KiB heap buffer; EINTR is retried, partial writes are looped. `-p` builds a `struct timespec[2]` from `st_atime`/`st_mtime` and `utimensat(AT_FDCWD, dst, &ts, 0)` (raw `syscall(280, ...)` pending [[cyrius-at-family-proposal]]). tty detection inline via `ioctl(fd, TCGETS, ...)`.
...
- **Tests**: `tests/kriya.tcyr` gains 17 octal-mode-parser assertions (78/78 passing total). `scripts/smoke-mkdir.sh` 24/24, `scripts/smoke-rmdir.sh` 24/24, `scripts/smoke-touch.sh` 26/26, `scripts/smoke-ln.sh` 30/30, `scripts/smoke-cp.sh` 26/26 (basic + 200KB multi-block, existing-dest gating with `-f`/`-i`, `-i`-on-pipe usage error, multi-into-dir, `-p` mode+mtime preservation, non-`-p` current-time, self-copy refusal, directory-without-R, partial-failure exit code, verbose). `tests/kriya.bcyr` gains `args/parse_octal_mode` at **20ns** steady-state.
