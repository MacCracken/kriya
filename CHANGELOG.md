...
#### M2 utility — mkdir (first M2 implementation)
...
- **Tests**: `tests/kriya.tcyr` gains 17 octal-mode-parser assertions (78/78 passing total). `scripts/smoke-mkdir.sh` covers behavioural cases (happy path, `-p` recursion, EEXIST split, `-m` mode bits, partial-failure exit code, root operand, multi-operand) — 24/24 passing. `tests/kriya.bcyr` gains `args/parse_octal_mode` at **20ns** steady-state.
