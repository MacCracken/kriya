...
- **Tests**: `tests/kriya.tcyr` gains 17 octal-mode-parser assertions (78/78 passing total). `scripts/smoke-mkdir.sh` covers behavioural cases (happy path, `-p` recursion, EEXIST split, `-m` mode bits, partial-failure exit code, root operand, multi-operand) — 24/24 passing. `scripts/smoke-rmdir.sh` covers happy path, `-p` cascade with sibling-content halt, `--ignore-fail-on-non-empty` silent halt, ENOTEMPTY/ENOENT/ENOTDIR error paths, EBUSY on `/`, and `-v` output — 24/24 passing. `scripts/smoke-touch.sh` covers create, multi-operand create, update-existing, `-a`/`-m` selectivity, `-a -m` together, `-c` on missing (exit 1) and existing (mtime bump), no-args usage error, ENOENT on missing parent, partial-failure exit code — 26/26 passing. `tests/kriya.bcyr` gains `args/parse_octal_mode` at **20ns** steady-state.
